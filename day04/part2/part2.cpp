...
int main()
{
	...
	struct { int id; int minute; int frequency; } mostFrequentSleep = { -1, -1, -1 };
	for (auto const& [id, schedule] : guardSchedules)
	{
		auto const mostFrequent = max_element(schedule.begin(), schedule.end());
		auto const frequency = *mostFrequent;
		if (frequency > mostFrequentSleep.frequency)
		{
			mostFrequentSleep.id = id;
			mostFrequentSleep.minute = static_cast<int>(distance(schedule.begin(), mostFrequent));
			mostFrequentSleep.frequency = frequency;
		}
	}

	auto const answer = mostFrequentSleep.id * mostFrequentSleep.minute;
