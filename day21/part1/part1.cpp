...
regex const instructionEx = regex(R"((\w+) (\d+) (\d+) (\d+))", regex::optimize);
...
int main()
{
	...
	auto ipValue = 0ULL;
	while (ipValue < instructions.size() && ipValue != 28)
	{
		reg[ip] = ipValue;
	...
	/*
	 * Solution was deduced manually from the input code. reg[0] is only checked for in instruction 28
	 * so once the program processed the register up to that instruction the answer must be whatever
	 * the equality check is matching against. I assume this to be the solution for all inputs.
	 */

	auto const answer = reg[instructions[28][1]];
